...
function showDashSection(sectionId, linkEl) {
  document
    .querySelectorAll("#page-patient-dashboard .dash-section")
  ...
  if (linkEl) {
    document
      .querySelectorAll("#page-patient-dashboard .sidebar-link")
...
function showDocSection(sectionId, linkEl) {
  document
    .querySelectorAll("#page-doctor-dashboard .dash-section")
    .forEach((s) => s.classList.remove("active"));
  const target = document.getElementById(sectionId);
  if (target) target.classList.add("active");
  ...
  if (linkEl) {
    document
      .querySelectorAll("#page-doctor-dashboard .sidebar-link")
      .forEach((l) => l.classList.remove("active"));
    linkEl.classList.add("active");
  }
}
...
function showAdminSection(sectionId, linkEl) {
  document
    .querySelectorAll("#page-admin-dashboard .dash-section")
    .forEach((s) => s.classList.remove("active"));
  const target = document.getElementById(sectionId);
  if (target) target.classList.add("active");
  ...
  if (linkEl) {
    document
      .querySelectorAll("#page-admin-dashboard .sidebar-link")
      .forEach((l) => l.classList.remove("active"));
    linkEl.classList.add("active");
  }
}
...
function switchTab(tabEl, contentId) {
  const parent =
    tabEl.closest(".dash-section") || tabEl.closest(".dash-card") || document;
...
function filterDoctors(specialty, btn) {
  if (btn) {
    document
      .querySelectorAll(".specialization-filter .filter-btn")
...
function resetBooking() {
  ...
  document
    .querySelectorAll(".doctor-card.selectable")
  ...
  document.querySelectorAll(".cal-day").forEach((d) => {
    if (
      ...
      !d.classList.contains("unavailable")
    ) {
...
function toggleDay(checkbox, day) {
  ...
  const badge = checkbox
    .closest(".avail-day-header")
    .querySelector(".avail-badge");
...
function cancelAppointment(btn) {
  openConfirm("Are you sure you want to cancel this appointment?", () => {
    const row = btn.closest("tr");
    if (row) {
...
const pharmacyData = {
  1: {
    ...
    address: "123 Health Ave, Suite 1",
  ...
  5: {
    ...
    address: "654 North Park Ave",
    phone: "+1 (555) 678-9012",
    hours: "Open 24/7",
  },
  6: {
    name: "QuickMeds Express",
    address: "987 East Side Lane",
...
function selectPharmacy(el, id) {
  ...
  // Update map pins
  document
    .querySelectorAll(".pharm-pin")
    .forEach((pin) => pin.classList.remove("active-pin"));
  ...
  if (data) {
    ...
    if (hoursEl)
      hoursEl.innerHTML = `<i class="fas fa-clock"></i> Hours: ${data.hours}`;
  }
...
const botResponses = {
  ...
  hi: { text: "Hi there! 👋 Welcome to Medix. How can I help?", action: null },
  cardiology: {
    text: "Our Cardiology department has 12 expert doctors. Shall I show you their profiles? 🫀",
    action: null,
  },
  dentistry: {
    text: "Our Dentistry team specializes in cosmetic, orthodontic, and surgical procedures. 🦷",
    action: null,
  },
  emergency: {
    text: "For emergencies, please call our hotline: +1 (800) 911-CARE or visit our ER immediately! 🚨",
    action: null,
  },
  payment: {
    text: "We accept online payments (credit/debit cards), cash at the clinic, and most major insurance plans. 💳",
    action: null,
  },
  insurance: {
    text: "We accept most major insurance plans. Please bring your insurance card when you visit. 📋",
    action: null,
  },
  contact: {
    text: "You can reach us at: 📞 +1 (800) 123-4567 or ✉️ care@medicarepro.com",
    action: null,
  },
  cancel: {
    text: "To cancel an appointment, go to My Appointments in your patient dashboard and click Cancel. Cancellations must be made 24 hours in advance.",
    action: null,
  },
  prescription: {
    text: "Your digital prescriptions are available in the Prescriptions section of your patient dashboard. 💊",
    action: null,
  },
  default: {
    text: "I'm not sure about that, but I'm here to help! You can ask me about: booking appointments, finding doctors, pharmacy locations, clinic hours, or payment options. 😊",
    action: null,
  },
};
